...
export default function EditAppointmentPage() {
  ...
  const fetchAppointmentDetails = async (id: string, token: string) => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/get_appointment/${id}`,
        {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/reschedule_appointment/${appointmentId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        setError(
          "Failed to reschedule appointment. Please check your inputs and try again."
        );
      }
    } catch (error) {
