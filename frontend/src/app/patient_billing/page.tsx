...
export default function PatientDashboard() {
  ...
  const fetchPatientData = async (patientId: number, token: string) => {
    ...
    try {
      ...
      const billingResponse = await fetch(`http://127.0.0.1:8000/patients/get_billing/${patientId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      if (billingResponse.ok) {
        ...
        console.error("Failed to fetch billing info:", await billingResponse.text())
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="border-2 border-black p-4 rounded-lg">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Link href="/patient_medical_history" className="border-2 border-black p-2 text-center rounded-lg hover:bg-gray-100">
              Medical History
            </Link>
          </div>
        </div>
              ...
              <tbody>
                {billingInfo.map((bill) => (
                  <tr key={bill.bill_id} className="border-b border-black">
                    ...
                    <td className="p-2">{formatCurrency(bill.amount + bill.tax)}</td>
                    <td className="p-2">
                      <span className={getPaymentStatusClass(bill.payment_status)}>{bill.payment_status}</span>
                    </td>
                    <td className="p-2">{bill.payment_method || "-"}</td>
                    <td className="p-2">{bill.payment_date ? formatDate(bill.payment_date) : "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
